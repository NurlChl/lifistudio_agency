...
function CategoryIcon({ category }: { category: string }) {
  if (category === "web") return <Globe size={14} />;
  if (category === "uiux") return <Smartphone size={14} />;
  if (category === "graphic") return <Palette size={14} />;
  if (category === "automation") return <Cpu size={14} />;
                  ...
                  >
                    ...
                    >
                      <div className="relative aspect-[16/10] overflow-hidden bg-stone-50">
                        {p.coverImage ? (
                          <img src={p.coverImage} alt={p.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
                        ) : (
                          <div className={cn("absolute inset-0 bg-gradient-to-br transition-all duration-700 group-hover:scale-105", style.gradient)}>
                            <div className={cn("absolute inset-0 opacity-30 transition-opacity duration-500 group-hover:opacity-10",
                              ...
                              style.pattern === "dots" && "bg-[radial-gradient(rgba(0,0,0,0.08)_1px,transparent_1px)] bg-[length:16px_16px]",
                        ...
                        <div className="absolute top-4 left-4">
                          <span className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold uppercase tracking-wider shadow-sm backdrop-blur-sm",
                            style.color === "stone" && "bg-white/80 text-stone-700",
                            style.color === "accent" && "bg-accent-500/80 text-white",
                            style.color === "purple" && "bg-accent-500/80 text-white",
                            style.color === "teal" && "bg-stone-900/70 text-white",
                          )}>
